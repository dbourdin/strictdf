...
class TestStrictDataFrame:
    def test_old_df_is_original_df(self):
        df = pd.DataFrame.from_dict({'column_1': [1, 2]})
        sdf = StrictDataFrame(df)
        pd.testing.assert_frame_equal(df, sdf.old_df)

    def test_new_df_is_created_with_valid_dict(self):
        df = pd.DataFrame.from_dict({'column_1': [1, 2]})
        sdf = StrictDataFrame(df)
        assert isinstance(sdf.new_df, pd.DataFrame)

    def test_not_using_dataframe_raises_exception(self):
        with pytest.raises(TypeError):
            StrictDataFrame('Not a pd.DataFrame')

    def test_parse_str_values_from_mixed_str_column(self):
        str_values = ['string_1', 'string_2', 'string_3']
        non_str_values = [5, 6]
        df = pd.DataFrame.from_dict(
            {'mixed_column': str_values + non_str_values}
        )

        sdf = StrictDataFrame(df)
        assert all([isinstance(value, str)
                    for value in sdf.new_df.mixed_column])

        assert all([any(value == sdf.new_df.mixed_column)
                    for value in str_values])

    def test_non_str_are_removed_from_mixed_str_column(self):
        str_values = ['string_1', 'string_2', 'string_3']
        non_str_values = [5, 6]
        df = pd.DataFrame.from_dict(
            {'mixed_column': str_values + non_str_values}
    ...
    def test_parse_int_values_from_mixed_int_column(self):
        int_values = ['1', '2', '3']
        non_int_values = ['string_1', 'string_2']
        df = pd.DataFrame.from_dict(
            {'mixed_column': int_values + non_int_values}
        )
    ...
    def test_non_int_are_removed_from_mixed_int_column(self):
        int_values = ['1', '2', '3']
        non_int_values = ['string_1', 'string_2']
        df = pd.DataFrame.from_dict(
            {'mixed_column': int_values + non_int_values}
    ...
    def test_parse_float_values_from_mixed_float_column(self):
    ...
    def test_non_float_are_removed_from_mixed_float_column(self):
        float_values = ['1.5', '2.5', '3.5']
        non_int_values = ['string_1', 'string_2']
        df = pd.DataFrame.from_dict(
            {'mixed_column': float_values + non_int_values}
    ...
    def test_parse_bool_values_from_mixed_bool_column(self):
    ...
    def test_all_parsed_bool_values_from_mixed_bool_column_are_bool(self):
        bool_values = [True, True, False]
        non_bool_values = ['string_1', 'string_2']
        df = pd.DataFrame.from_dict(
            {'mixed_column': bool_values + non_bool_values}
    ...
    def test_non_bool_are_removed_from_mixed_bool_column(self):
    ...
    def test_bool_are_parsed_from_int_bool_column(self):
    ...
    def test_bool_values_are_parsed_correctly_from_strings(self):
    ...
    def test_bool_is_removed_if_is_not_int_bool_column(self):
        values = [1, 0, 0, 2, True]
        df = pd.DataFrame.from_dict({'int_column': values})
        sdf = StrictDataFrame(df)
        assert not any([type(value) == bool
                        for value in sdf.new_df.int_column])

    def test_report_is_displayed_properly(self, capsys):
        keep_values = [1, 2, 3]
    ...
    def test_can_retrieve_dtypes_dict_from_strict_data_frame(self):
        values = [1, 0, 0, 2, True]
        df = pd.DataFrame.from_dict({'int_column': values})
        sdf = StrictDataFrame(df)
    ...
    def test_dtypes_are_generated_correctly_for_each_column(self):
